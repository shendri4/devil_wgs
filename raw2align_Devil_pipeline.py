...
for sample in samples:
    ...
    # DStreett ran SD on files on petunia(or whale) because required 0.5Tb of RAM?
...
#ln -s file /mnt/lfs2/stre3949/Sarah/filename .
    ...
    cmd = ' '.join(['flash2 --max-overlap 150 --allow-outies --threads 7 -o', jp(resultsDir, sample + '_flash'),
                    '-d', resultsDir, jp(rawdataDir, sample + '.fastq.1.gz'), jp(rawdataDir, sample + '.fastq.2.gz'),
                    '>>', logFile, '2>&1'])
    log(cmd, logCommands)
